...
const apiData = {
  ...
  languages: [
    ...
    { id: 'es-es', name: 'Spanish - Spain (Traditional)' },
    { id: 'fi', name: 'Finnish' },
  ...
  forms: {
    help: {
      ...
      action: 'https://srenault.cdn.prismic.io/api/v1/documents/search',
      fields: {
        ref: { type: 'String', multiple: false },
        q: {
          default:
            '[[:d = at(document.tags, ["help"])][:d = any(document.type, ["article"])]]',
          type: 'String',
          multiple: true,
        },
    ...
    toto: {
      ...
      fields: {
        ref: { type: 'String', multiple: false },
        q: { default: '[]', type: 'String', multiple: true },
        lang: { type: 'String', multiple: false },
        page: { type: 'Integer', multiple: false, default: '1' },
        pageSize: { type: 'Integer', multiple: false, default: '20' },
        after: { type: 'String', multiple: false },
  ...
  oauth_initiate: 'https://srenault.prismic.io/auth',
  oauth_token: 'https://srenault.prismic.io/auth/token',
  version: 'aa1901d',
  ...
  experiments: {
    ...
    running: [
      {
        id: 'WMK1pioAACkA6AOf',
        googleId: 'GZRjt1AKQMe00KygORk8zg',
        name: 'experiment',
        variations: [
          {
            id: 'WMK1pioAACsA6AOh',
            ref: 'WMK1pioAAOgB6AOi~XcGL-REAACYA2Su_',
            label: 'Base',
          },
          {
            id: 'WMK1uSoAACsA6AP9',
            ref: 'WMK1uSoAABEA6AP-~XcGL-REAACYA2Su_',
            label: 'variationA',
          },
        ],
      },
...
describe('PrismicLink', () => {
  ...
  it('enforces cdn for prismic endpoint', (done) => {
    ...
    execute((link as unknown) as ApolloLink, {
      ...
      next: makeCallback(done, () => {
        ...
        expect(uri).toBe(
          'https://test.cdn.prismic.io/graphql?query=%7B_allDocuments%7Bedges%7Bnode%7B_meta%7Bid%7D%7D%7D%7D%7D&variables=%7B%7D'
        )
      }),
  ...
  it('do not do anything for non prismic endpoint', (done) => {
    ...
    execute((link as unknown) as ApolloLink, {
      ...
      next: makeCallback(done, () => {
        ...
        expect(headers['Prismic-ref'] === apiData.refs[0].ref)
        expect(uri).toBe(
          'https://my.website.com/graphql?query=%7B_allDocuments%7Bedges%7Bnode%7B_meta%7Bid%7D%7D%7D%7D%7D&variables=%7B%7D'
        )
      }),
      error: (error) => done.fail(error),
    })
  ...
  it('throws for non prismic endpoint and missing repositoyName field', () => {
    expect(() => {
      PrismicLink({
        uri: 'https://my.website.com/graphql',
      })
